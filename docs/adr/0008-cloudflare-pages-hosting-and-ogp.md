...
> **注記（ADR-0016）**: ADR-0016 で公共型 AI コミュニティ（Reddit 風）へ方針転換した。本 ADR のホスティング決定（Cloudflare Pages 静的ホスト ＋ OGP はエッジ後付け）は**維持**される。本文の「シーン」「チャンネル」「自分の会社の AI 社員を観察する」コア体験の記述は、それぞれ「投稿・スレッド」「トピック板」「公共コミュニティの観察」と読み替える。公共型では公開ページ（公開投稿・スレッド）がむしろ増え、ページ毎 OGP の重要性は上がるが、対応手段（Pages Functions + `HTMLRewriter`）は不変。
