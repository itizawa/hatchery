...
// #1163: 投稿・コメント本文中の他ワーカーの表示名をプロフィールへの自動リンクにする。
describe("PostThreadScene — 本文中のワーカー名がプロフィールへのリンクになる (#1163)", () => {
  it("コメント本文中に含まれる post 著者の表示名がプロフィールへのリンクとして描画される", async () => {
    server.use(
      http.get("/api/posts/:postId", () =>
        HttpResponse.json({
          post: { ...mockPosts[0], author_worker: { id: "worker-uuid-haru", display_name: "はる", image_url: null } },
    ...
    // コメント自体の描画完了を、本文分割の影響を受けない要素（コメント著者名）で待つ。
    await screen.findByText("けん");
    // コメント本文（"はるさんに同意です"）内で「はる」だけがリンク化されている。
    // byline 側の「はる」（post 著者名）・「けん」（コメント著者名）とは別に、
    // コメント本文の段落内にちょうど 1 つ「はる」テキストを持つ <a> が存在することを確認する。
    const commentWrapper = container.querySelector("#comment-comment-1");
    expect(commentWrapper).not.toBeNull();
    const mentionLinks = Array.from(commentWrapper?.querySelectorAll("a") ?? []).filter(
      (a) => a.textContent === "はる",
    );
    expect(mentionLinks).toHaveLength(1);
  });

  it("本文中に既知ワーカーへの言及が無いときはリンク化されない", async () => {
    server.use(
      http.get("/api/posts/:postId", () =>
        HttpResponse.json({
          post: { ...mockPosts[0], author_worker: { id: "worker-uuid-haru", display_name: "はる", image_url: null } },
    ...
    await screen.findByText("誰にも言及していない本文です");
    const commentWrapper = container.querySelector("#comment-comment-1");
    const mentionLinks = Array.from(commentWrapper?.querySelectorAll("a") ?? []).filter(
      (a) => a.textContent === "はる",
    );
