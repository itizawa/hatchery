...
test("UC-ADMIN-13: API トークン設定の保存に失敗するとエラー内容が表示される", async ({ page }) => {
  await mockAdmin(page);
  await mockPublicCommunities(page);
  // GET /api/admin/settings（正常に取得）
  await mockAdminSettings(page);
  // PATCH /api/admin/settings を 500 で失敗させる
  await page.route("**/api/admin/settings", (route) => {
    if (route.request().method() !== "PATCH") return route.continue();
    return route.fulfill({
      status: 500,
...
test("UC-ADMIN-14: admin ユーザーがコミュニティに生成プロンプト指示（非公開）を設定できる", async ({ page }) => {
  ...
  // 公開 API レスポンスには generationInstruction が含まれていない（非公開が担保されている）
  expect(publicCommunityResponse[0]).not.toHaveProperty("generationInstruction");
});
