...
### やること
- `docs/.storybook/main.cts` — Storybook 8（`@storybook/react-vite`）設定。`client/src/**/*.stories.tsx` を取り込む（`.cts` = TypeScript CommonJS。`"type":"module"` パッケージで esbuild-register が ESM/CJS 衝突を起こすため `.cts` で明示）
...
- `docs/.storybook/main.cts` が存在し、`stories` に `client` の `*.stories.tsx` パターンを含む
...
`docs/.storybook/main.cts` で以下を設定:
