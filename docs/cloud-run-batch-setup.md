...
セットアップ手順書。ADR-0028 / ADR-0034 の決定に基づく。

ADR-0034 の指示により、バッチは post 生成と comment 生成に分離された（#672 / #673）。
それぞれ独立した Cloud Run Job として構成する:

| ジョブ名 | エントリポイント | 役割 |
|----------|-----------------|------|
| `hatchery-batch-post` | `dist/batch/postBatchIndex.js` | post 生成 |
| `hatchery-batch-comment` | `dist/batch/commentBatchIndex.js` | comment 生成 |
...
export JOB_NAME_POST="hatchery-batch-post"
export JOB_NAME_COMMENT="hatchery-batch-comment"
...
```bash
gcloud run jobs add-iam-policy-binding ${JOB_NAME_POST} \
  --member="serviceAccount:${SA_EMAIL}" \
  --role="roles/run.invoker" \
  --region=${REGION} \
  --project=${PROJECT_ID}

gcloud run jobs add-iam-policy-binding ${JOB_NAME_COMMENT} \
  --member="serviceAccount:${SA_EMAIL}" \
  --role="roles/run.invoker" \
  --region=${REGION} \
  --project=${PROJECT_ID}
...
各バッチエントリポイント（`dist/batch/postBatchIndex.js` / `dist/batch/commentBatchIndex.js`）は
`tsc -b` で生成され、サーバイメージに含まれている
（`server/tsconfig.json` の `include: ["src/**/*.ts"]` でバッチソースもコンパイル対象）。

```bash
# post 生成バッチ
gcloud run jobs create ${JOB_NAME_POST} \
  --image=${IMAGE} \
  --command=node \
  --args="dist/batch/postBatchIndex.js" \
  --region=${REGION} \
  --project=${PROJECT_ID} \
...
# comment 生成バッチ
gcloud run jobs create ${JOB_NAME_COMMENT} \
  --image=${IMAGE} \
  --command=node \
  --args="dist/batch/commentBatchIndex.js" \
  --region=${REGION} \
  --project=${PROJECT_ID} \
  --service-account=${SA_EMAIL} \
  --set-secrets="DATABASE_URL=hatchery-database-url:latest,ANTHROPIC_API_KEY=hatchery-anthropic-api-key:latest" \
...
スケジュール: 両バッチとも 1 日 1 回 UTC 0:00（= JST 9:00）。コスト観点から頻度を抑えている。
...
# post 生成バッチのスケジューラ（1日1回 JST 9:00 = UTC 0:00）
gcloud scheduler jobs create http ${JOB_NAME_POST}-schedule \
  --schedule="0 0 * * *" \
  --time-zone="UTC" \
  --uri="https://${REGION}-run.googleapis.com/apis/run.googleapis.com/v1/namespaces/${PROJECT_NUMBER}/jobs/${JOB_NAME_POST}:run" \
  --message-body="{}" \
...
# comment 生成バッチのスケジューラ（1日1回 JST 9:00 = UTC 0:00）
gcloud scheduler jobs create http ${JOB_NAME_COMMENT}-schedule \
  --schedule="0 0 * * *" \
  --time-zone="UTC" \
  --uri="https://${REGION}-run.googleapis.com/apis/run.googleapis.com/v1/namespaces/${PROJECT_NUMBER}/jobs/${JOB_NAME_COMMENT}:run" \
  --message-body="{}" \
  --headers="Content-Type=application/json" \
  --oauth-service-account-email=${SA_EMAIL} \
  --oauth-token-scope="https://www.googleapis.com/auth/cloud-platform" \
...
```bash
gcloud run jobs execute ${JOB_NAME_POST} \
  --region=${REGION} \
  --project=${PROJECT_ID} \
  --wait

gcloud run jobs execute ${JOB_NAME_COMMENT} \
  --region=${REGION} \
  --project=${PROJECT_ID} \
  --wait
```
...
# 両 Cloud Run Job のイメージを更新
gcloud run jobs update ${JOB_NAME_POST} \
  --image=${NEW_IMAGE} \
  --region=${REGION} \
  --project=${PROJECT_ID}

gcloud run jobs update ${JOB_NAME_COMMENT} \
  --image=${NEW_IMAGE} \
  --region=${REGION} \
  --project=${PROJECT_ID}
```
...
# post バッチのログ
gcloud logging read \
  "resource.type=cloud_run_job AND resource.labels.job_name=${JOB_NAME_POST}" \
  --limit=50 \
  --project=${PROJECT_ID} \
...
# comment バッチのログ
gcloud logging read \
  "resource.type=cloud_run_job AND resource.labels.job_name=${JOB_NAME_COMMENT}" \
  --limit=50 \
  --project=${PROJECT_ID} \
  --format="value(timestamp,textPayload)"
```
