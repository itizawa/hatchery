...
test("ヘッダーのログイン導線からログインモーダルが開く（#454） UC-AUTH-01", async ({ page }) => {
...
test("Google でログインすると Google OAuth へリダイレクトされる（#455） UC-AUTH-02", async ({
  page,
}) => {
  await mockUnauthenticated(page);
  await mockContentApis(page);
...
test("Google OAuth 完了後にホームへリダイレクトされる UC-AUTH-03", async ({ page }) => {
...
test("ログアウトすると未ログイン状態に戺る UC-AUTH-04", async ({ page }) => {
  // 認証状態を動的に切り替えるクロージャ
  ...
  // 未ログイン状態に戺る: ヘッダーに「ログイン」リンクが表示される
...
test(
  "未ログインで認証必須ページ（/account）にアクセスするとホーム上にログインモーダルが開く（#454） UC-AUTH-05",
  async ({ page }) => {
    await mockUnauthenticated(page);
    await mockContentApis(page);
...
test(
  "未ログインで管理画面（/admin）にアクセスするとホーム上にログインモーダルが開く（#454） UC-AUTH-06",
  async ({ page }) => {
    await mockUnauthenticated(page);
    await mockContentApis(page);
...
test(
  "旧 /login URL はホーム上のログインモーダルへ誘導される（#454 後方互換） UC-AUTH-07",
  async ({ page }) => {
    await mockUnauthenticated(page);
    await mockContentApis(page);
