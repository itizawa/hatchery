...
- **`milestone/*` ラベルが無い Issue は自動選択の対象外**（着手しない）。`priority/*` 等が付いていても、`milestone/*` ラベルが無ければ引数なし `/df` では選ばれない（STEP 0 の表で「マイルストーン未設定＝自動選択対象外」と報告するのみ）。
...
取得した一覧を各 Issue の `df:*` / `priority/*` ラベル・**`milestone/*` ラベル**で分類し、**最初に状況テーブルを必ず提示**する（これは最終報告にもそのまま使う）:

| # | タイトル | df ラベル | 優先度 | マイルストーン | 次の担当 | AI 実行可能 |
|---|----------|-----------|--------|----------------|----------|:----------:|

- **AI 実行可能**（着手対象）= `df:todo` / `df:dev-review` のいずれか **かつ** `df:done` / `df:blocked` をいずれも持たない。
  - ※ ただし**引数なし自動選択（STEP 1-B）ではさらに `milestone/*` ラベルが付いていることを要する**（後述「マイルストーン」）。「AI 実行可能」列は df ラベルだけで判定し、`milestone/*` 未設定で自動選択対象外になるものは備考でその旨を添える。
- **人間の番**（着手しない）= `df:done` / `df:blocked` を 1 つでも持つ。「👤 人間」と明記。
- `df:*` ラベル無し = 対象外（「未分類」として軽く触れる）。
- `milestone/*` ラベル無し（`ms=-`）の Issue は、表に「(マイルストーン未設定＝自動選択対象外)」と明記する。
...
対象は **AI 実行可能ラベル**（`df:todo` / `df:dev-review`）のいずれかを持ち、**かつ `df:done` / `df:blocked` を持たず**、**かつ `milestone/*` ラベルが付いている** Open Issue のみ。

1. **候補集合のフィルタ**: Open Issue から、(a) `df:done` / `df:blocked` を 1 つでも含む Issue、(b) **`milestone/*` ラベルが付いていない Issue** を**除外**する。残った中で AI 実行可能 df ラベルを持つものだけを候補にする。
   - フィルタの結果、AI 実行可能だが `milestone/*` ラベル未設定で除外された Issue があれば、STEP 3 報告でその旨（「マイルストーンを設定（GitHub Actions が自動的にラベルを付与）すれば自動選択対象になる」）を添える。

2. **優先順位**（上から順に評価し、最初に 1 件決まったら確定。**先のキーで差がついたら後のキーは見ない**）:
   1. **`milestone/*` ラベルのアルファベット昇順**（前述「マイルストーン」）: 候補の `milestone/*` ラベルを辞書順で並べ、**アルファベット的に最小のラベルを持つ Issue を優先**する。
      - バージョン命名規則（`v1.0.0`, `v1.0.1`, `v1.1.0` ...）ではアルファベット順が締切の早い順と一致する。
      - `milestone/*` が複数付いている場合は最小値（最も早いマイルストーン）を採用する。
      - （候補は全て `milestone/*` ラベル設定済みなので、ここで「未設定」は出てこない＝手順 1 で除外済み。）
...
決定したら、**選んだ Issue 番号・現ラベル・実行フェーズ・`milestone/*` ラベル・選定理由**を一言で宣言してから着手する。
`milestone/*` ラベル付きの AI 実行可能 Issue が 1 件も無ければ、STEP 0 の表とともに「現在 AI の番の Issue はありません（全て人間ゲート待ち / マイルストーン未設定 / Open Issue 無し）」と報告して終了。**`milestone/*` ラベル未設定で対象外になった AI 実行可能 Issue がある場合は、その旨（マイルストーンを設定すれば GitHub Actions が自動的にラベルを付与し、次回から自動選択される）を必ず添える。**
...
1. **状況サマリ表**（STEP 0 の「誰の番か」分類テーブル: # / タイトル / df ラベル / 優先度 / マイルストーン（`milestone/*` ラベル） / 次の担当 / AI 実行可能）。`milestone/*` ラベル未設定で自動選択対象外になった AI 実行可能 Issue があれば併記する。
...
6. **AI 実行可能な残候補**（あれば自動選択順＝`milestone/*` ラベル昇順→優先度→フェーズ→古い順。`/loop /df` や `/schedule` で連続実行する人間向け）。**`milestone/*` ラベル未設定で対象外の AI 実行可能 Issue**も別掲し、「マイルストーンを設定すれば GitHub Actions が自動的にラベルを付与し自動選択される」と添える。任意で **branch protection 設定の推奨**を添える。
