...
- `postRepository.getStatsByCommunity()`（既存・community 別 postCount 再利用）
- `commentRepository.count()`
- `voteRepository.count()`
- `subscriptionRepository.subscriberCountPerCommunity()`（既存・再利用）
- `viewRepository.viewCountByCommunity()`

**`communityRepository.count()` / `postRepository.count()` / `subscriptionRepository.count()` /
`viewRepository.totalViewCount()` は route から呼ばない**（セルフレビューで指摘・修正）。
post/comment/subscription/view は必ず community に紐づくため、内訳集計用に取得済みの
community 一覧・Map の総和がそのまま全体数と一致する（`community_count = communities.length`、
`post_count = Σ postStatsByCommunity`、`total_subscription_count = Σ subscriberCounts`、
`total_view_count = Σ viewCountsByCommunity`）。この 4 つの `count()`/`totalViewCount()` メソッド自体は
受け入れ条件どおり interface・in-memory・Prisma 実装とテストを備えるが、認証不要の公開エンドポイントで
毎リクエスト発生する冗長な DB ラウンドトリップを避けるため、route はこれらを呼ばず既存の内訳データから導出する。
（`workerRepository.count()` / `commentRepository.count()` / `voteRepository.count()` は内訳データに
対応する Map が存在しないため、独立した集計として残す。）
...
- `viewCountByCommunity()`: Post と Comment はいずれも `communityId` を直接持つため、`viewsByWorkerSince`
  のような raw SQL（`PageView` 経由の author 解決が必要な場合のみ使う作法）ではなく、
  `prisma.post.groupBy({ by: ["communityId"], _sum: { viewCount: true } })` /
  `prisma.comment.groupBy(...)` を並列実行し JS 側で合算する（セルフレビューで raw SQL → 既存の
  `groupBy` 慣用パターンへ簡素化）。
...
任意コールバック `resolveCommunity` を `createInMemoryViewRepository` に追加する。CLAUDE.md の関数引数規約
（#720）に従い、`resolveAuthor` / `clock` / `resolveCommunity` の 3 つはオブジェクト引数
（`{ resolveAuthor, clock, resolveCommunity }`）にまとめる（セルフレビューで、位置引数を 3 つに拡張する
案から規約準拠のオブジェクト引数へ修正）。既存の 0 引数呼び出し元（`createTestDeps.ts` 等）はデフォルト値
`{}` により無変更で動作する。
