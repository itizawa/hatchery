...
import { InMemoryCommunityRepository } from "./communityRepository.js";

const makeCommunity = (overrides: Partial<Parameters<typeof InMemoryCommunityRepository>[0][0]> = {}) => ({
  id: "community-1",
  slug: "technology",
  name: "Technology",
...
describe("InMemoryCommunityRepository", () => {
  describe("findById", () => {
    it("存在する id で取得できる", async () => {
      ...
      const repo = new InMemoryCommunityRepository([community]);
  ...
  describe("list", () => {
    it("全コミュニティを createdAt 昇順で返す", async () => {
      ...
      const repo = new InMemoryCommunityRepository(communities);
