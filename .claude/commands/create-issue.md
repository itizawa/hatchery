---
description: Dark Factory ワークフロー向けに GitHub Issue を起票する。ユーザーの要望（$ARGUMENTS）から、concept.md・ADR・既存コードを踏まえて「背景 / 目的 / 受け入れ条件（テストに落とせる粒度）/ 補足」構成の Issue 本文を起こし、priority/* ラベル + マイルストーンを付けて gh issue create する。マイルストーンの指示が無ければ聞き返す。実装はしない（起票のみ）。
...
2. **マイルストーンの決定**:
   ...
   - **指示が無い場合は必ず聞き返す**（推測で勝手に決めない／未設定で起票しない）。上記で取得した既存マイルストーン一覧（タイトル・due）を提示し、「どのマイルストーンに紐づけますか？」とユーザーに尋ねて**回答を待つ**。
     - ユーザーが「直近のでいい」等と言えば、due_on が最も近い open マイルストーンを採用する。
     - 一覧に該当が無く新規作成が必要な場合は、作成してよいか（タイトル・due_on）を確認してから `gh api repos/{owner}/{repo}/milestones --method POST -f title=... -f due_on=...` で作成する。**勝手に新規マイルストーンを作らない**。
...
- **ただしマイルストーンの指示が無い場合は「提示→即作成」をせず、必ず STEP 2 の手順で聞き返してから作成する**（マイルストーンは未確定のまま起票しない）。
...
- ❌ 状態ラベル（`df:dev-review` / `df:done` / `df:blocked`）を起票時に付ける。
- ❌ ADR / concept.md / MVP 制約に反する内容を、断り書きなく受け入れ条件に入れる。
- ❌ 受け入れ条件をテスト不能な曖昧表現のままにする。
- ❌ 既存 Issue と重複する内容を確認せず作る。
- ❌ マイルストーンの指示が無いのに聞き返さず、推測でマイルストーンを決める／未設定のまま起票する（ユーザーが明示的に「無しで」と言った場合を除く）。
