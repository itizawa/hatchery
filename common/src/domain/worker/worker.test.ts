...
describe("resolveWorkerImageUrl (#1015)", () => {
  ...
  it("imageUrl が null のとき null を返す（URL を捏造しない・#1015）", () => {
    expect(resolveWorkerImageUrl({ id: "haru", imageUrl: null })).toBeNull();
  });

  it("imageUrl が undefined のとき null を返す（#1015）", () => {
    expect(resolveWorkerImageUrl({ id: "haru", imageUrl: undefined })).toBeNull();
  });

  it("imageUrl を省略したとき null を返す（#1015）", () => {
    expect(resolveWorkerImageUrl({ id: "ken" })).toBeNull();
  });
});
