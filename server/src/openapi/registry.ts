...
registry.registerPath({
  ...
  responses: {
    ...
    400: { description: "バリデーションエラー（uuid 不正セtitle/text 空など）", ...errorJson },
    401: { description: "未認証", ...errorJson },
    403: { description: "admin 権限なし", ...errorJson },
    404: { description: "community / worker（削除済を含む）が存在しない", ...errorJson },
  },
});
...
registry.registerPath({
  ...
  responses: {
    ...
    400: { description: "バリデーションエラー（uuid 不正セtext 空など）", ...errorJson },
    401: { description: "未認証", ...errorJson },
    403: { description: "admin 権限なし", ...errorJson },
    404: { description: "post / worker（削除済を含む）が存在しない", ...errorJson },
  },
});
