...
| 3 | キャッシュキーが正しく機能し入力変更タスクは再実行 | `turbo.json` の各タスク `outputs` を点検。`lint`/`test` は出力レス（成否のみキャッシュ）で妥当、`build` 系は `dist/**` 等を網羅。`globalEnv`/`globalDependencies` でロックファイル等を入力に含める（turbo は既定で root `pnpm-lock.yaml` 等をグローバル入力に含むが、明示する）。 |
