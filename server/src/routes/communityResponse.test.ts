...
describe("toCommunityResponse", () => {
  it("全フィールドを snake_case の公開 API 形式に変換する", () => {
    const res = toCommunityResponse({ r: baseRecord });
    expect(res).toEqual({
      id: "community-1",
      slug: "general",
      name: "General",
  ...
  it("stats を渡すと post_count・last_post_at が反映される（#527）", () => {
    ...
    const res = toCommunityResponse({ r: baseRecord, stats });
