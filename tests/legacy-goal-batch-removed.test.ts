...
const REMOVED_BATCH_FILES = [
  ...
  // ADR-0034 (#901): communityBatchIndex は postBatchIndex / commentBatchIndex に分離済みのため削除。
  "server/src/batch/communityBatchIndex.ts",
  "server/src/batch/communityBatchIndex.test.ts",
  "server/src/batch/runCommunityBatch.ts",
  "server/src/batch/runCommunityBatch.test.ts",
] as const;
...
describe("ADR-0034 (#901): post/comment 分割バッチの実装は維持されている", () => {
  it("server/src/batch/postBatchIndex.ts が存在する", () => {
    expect(pathExists("server/src/batch/postBatchIndex.ts")).toBe(true);
  });

  it("server/src/batch/commentBatchIndex.ts が存在する", () => {
    expect(pathExists("server/src/batch/commentBatchIndex.ts")).toBe(true);
  });
});
