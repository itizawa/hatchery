...
## UC-PWA-07: ホームフィードでスナックバーインストール導線が表示される

**対象画面**: ホームフィード（`/`）・スナックバー

**前提条件**:
- Chrome（Android / デスクトップ）でアプリを開く（`beforeinstallprompt` イベントが発火する環境）
- アプリがまだインストールされていない
- スナックバーを dismiss した履歴がない

**ステップ**:
1. ホームフィード（`/`）を開く
2. 投稿カードを 3 件以上スクロールして視野に入れる
3. 画面下部を確認する

**期待動作**:
- 「ホーム画面に追加してすばやく起動」のスナックバーが画面下部に表示される
- 「追加する」ボタンを押すと Chrome のインストールプロンプトが表示される
- 「×」（閉じる）ボタンを押すとスナックバーが消え、以後同一ブラウザで再表示されない

---

## UC-PWA-08: ヘッダーのインストールアイコンボタンが常設される

**対象画面**: グローバルヘッダー（全ページ共通）
...
**ステップ**:
1. 任意のページ（ホームフィード・投稿スレッド等）を開く
2. ヘッダー右側（検索アイコンの隣）を確認する

**期待動作**:
- ダウンロード（GetApp）アイコンボタンが検索アイコンの右隣に表示される
- ボタンを押すと Chrome のインストールプロンプトが表示される（iOS の場合はホーム画面追加の案内 Dialog が開く）
- アプリインストール後または既インストール状態ではボタンが非表示になる
