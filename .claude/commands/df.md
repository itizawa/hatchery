...
引数なし自動選択（STEP 1-B）で **どの Issue を先に着手するか**を決める**最上位キー**であり、同時に**着手対象の絞り込み条件**でもある。`priority/*` より優先する。引数で番号指定したとき（STEP 1-A）は使わない（人間が明示選択したものを尊重する＝マイルストーン未設定でも着手する）。

- **マイルストーン未設定の Issue は自動選択の対象外**（着手しない）。`priority/*` 等が付いていても、マイルストーンが無ければ引数なし `/df` では選ばれない（STEP 0 の表で「マイルストーン未設定＝自動選択対象外」と報告するのみ）。
- **「直近のマイルストーン」= due_on（締切）が最も近いもの**。候補 Issue が属するマイルストーンを due_on 昇順で並べ、**最も早い due_on のマイルストーンに属する Issue を最優先**する。
- **due_on が設定されていないマイルストーン**は「直近」の比較で**末尾扱い**（due_on を持つマイルストーンすべてより後）。複数の due_on 無しマイルストーン間はタイトル昇順で安定化する。
- マイルストーンは着手順と対象絞り込みを決めるだけで、`df:done` / `df:blocked`（人間の番）の判定を上書きしない（ゲート 6 が常に優先）。
...
5. Open Issue を一覧化（**マイルストーン情報も取得する**）:
   ...
   gh issue list --state open --limit 100 \
     ...
     -q 'sort_by(.createdAt)[] | "\(.number)\t\(.title)\t[\([.labels[].name] | join(","))]\tms=\(.milestone.title // "-")(due=\(.milestone.dueOn // "-"))"'
   ```

取得した一覧を各 Issue の `df:*` / `priority/*` ラベル・**マイルストーン**で分類し、**最初に状況テーブルを必ず提示**する（これは最終報告にもそのまま使う）:

| # | タイトル | df ラベル | 優先度 | マイルストーン(due) | 次の担当 | AI 実行可能 |
|---|----------|-----------|--------|---------------------|----------|:----------:|

- **AI 実行可能**（着手対象）= `df:todo` / `df:dev-review` のいずれか **かつ** `df:done` / `df:blocked` をいずれも持たない。
  - ※ ただし**引数なし自動選択（STEP 1-B）ではさらにマイルストーン設定済みであることを要する**（後述「マイルストーン」）。「AI 実行可能」列は df ラベルだけで判定し、マイルストーン未設定で自動選択対象外になるものは備考でその旨を添える。
- **人間の番**（着手しない）= `df:done` / `df:blocked` を 1 つでも持つ。「👤 人間」と明記。
- `df:*` ラベル無し = 対象外（「未分類」として軽く触れる）。
- マイルストーン未設定（`ms=-`）の Issue は、表に「(マイルストーン未設定＝自動選択対象外)」と明記する。
...
対象は **AI 実行可能ラベル**（`df:todo` / `df:dev-review`）のいずれかを持ち、**かつ `df:done` / `df:blocked` を持たず**、**かつマイルストーンが設定されている** Open Issue のみ。

1. **候補集合のフィルタ**: Open Issue から、(a) `df:done` / `df:blocked` を 1 つでも含む Issue、(b) **マイルストーンが未設定（`ms=-`）の Issue** を**除外**する。残った中で AI 実行可能 df ラベルを持つものだけを候補にする。
   - フィルタの結果、AI 実行可能だがマイルストーン未設定で除外された Issue があれば、STEP 3 報告でその旨（「マイルストーンを設定すれば自動選択対象になる」）を添える。

2. **優先順位**（上から順に評価し、最初に 1 件決まったら確定。**先のキーで差がついたら後のキーは見ない**）:
   1. **直近マイルストーンの due_on 昇順**（前述「マイルストーン」）: 候補が属するマイルストーンを締切（`dueOn`）の早い順に並べ、**最も早い due_on のマイルストーンに属する Issue を優先**する。
      - **due_on が無いマイルストーン**は末尾扱い（due_on を持つマイルストーンすべてより後）。due_on 無し同士はマイルストーンタイトル昇順。
      - （候補は全てマイルストーン設定済みなので、ここで「未設定」は出てこない＝手順 1 で除外済み。）
   2. 同マイルストーン内では **優先度ラベルの重み降順**（前述「優先度ラベル」表）: `priority/critical`(4) ＞ `priority/high`(3) ＞ `priority/medium`(2) ＞ `priority/low`(1)。
      - **`priority/*` が無い Issue は重み 2（medium）**として比較に参加させる（未設定でも low に負けない）。
      - `priority/*` が複数付いていたら**最も高い重み**を採用する。
   3. 同優先度内では **「進行中の作業を先に終わらせる」**（フェーズ進捗度の降順）: `df:dev-review`（あと一歩でマージ）＞ `df:todo`（起点）。
      - 根拠: 仕掛かり（WIP）を減らすほどリードタイムが短くなり、未マージの実装 PR が古びてコンフリクトする事故を防げる。「終わりに近いものから片付ける」。
   4. それでも同点なら **`createdAt` が古い順**（FIFO。滞留 Issue の飢餓＝永久放置を防ぐ）。
...
決定したら、**選んだ Issue 番号・現ラベル・実行フェーズ・属するマイルストーン(due)・選定理由**を一言で宣言してから着手する。
（マイルストーン設定済みの）AI 実行可能 Issue が 1 件も無ければ、STEP 0 の表とともに「現在 AI の番の Issue はありません（全て人間ゲート待ち / マイルストーン未設定 / Open Issue 無し）」と報告して終了。**マイルストーン未設定で対象外になった AI 実行可能 Issue がある場合は、その旨（マイルストーンを設定すれば自動選択される）を必ず添える。**
...
1. **状況サマリ表**（STEP 0 の「誰の番か」分類テーブル: # / タイトル / df ラベル / 優先度 / マイルストーン(due) / 次の担当 / AI 実行可能）。マイルストーン未設定で自動選択対象外になった AI 実行可能 Issue があれば併記する。
...
6. **AI 実行可能な残候補**（あれば自動選択順＝直近マイルストーン→優先度→フェーズ→古い順。`/loop /df` や `/schedule` で連続実行する人間向け）。**マイルストーン未設定で対象外の AI 実行可能 Issue**も別掲し、「マイルストーンを設定すれば自動選択される」と添える。任意で **branch protection 設定の推奨**を添える。
...
引数あり?
 ...
 └ No  → 候補から done/blocked と「マイルストーン未設定」を除外 →
            直近マイルストーン(due_on昇順, due無しは末尾) → 優先度(critical>high>medium=無印>low) →
