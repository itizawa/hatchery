...
| admin | 管理画面（Worker / Community 管理, `/admin`）・Worker 一覧ページネーション（#545）・トークン使用量コスト表示・日別コストグラフ（#664）・Worker 作成・編集専用ページ（#888）・コミュニティ作成・編集専用ページ（#889）・コミュニティ単位の生成停止トグル（#1011）・画像アップロード失敗時のエラーフィードバック（#1026） | [admin/usecases.md](admin/usecases.md) | UC-ADMIN-01～28 |
