/**
 * MVP の静的な発言テンプレート（#32）。ワーカー id ごとの文言リスト。
 * 定時バッチが「複数ワーカー × ランダム選択」で発言を組み立てる際の素材として使う。
 * AI による生成への移行は別 Issue（#53）。それまでの単一情報源として common に置く（ADR-0005）。
 *
 * id は DEFAULT_WORKERS（haru / ken / mei）と整合させる。各文言は MessageSchema を満たすよう
 * 非空かつ MAX_MESSAGE_LENGTH 以内に収める。
 */
export const WORKER_MESSAGE_TEMPLATES: Readonly<Record<string, readonly string[]>> = {
  haru: [
    "おはようございます！今日も一日がんばりましょう。",
    "いい天気ですね、気分が上がります。",
    "ちょっと休桬しませんか？コーヒーでもどうぞ。",
    "みんなのおかげで今日も機しいです！",
  ],
  ken: [
