...
export class PrismaTokenUsageLogRepository implements TokenUsageLogRepository {
  constructor(private readonly prisma: PrismaClient) {}

  async create(entry: Omit<TokenUsageLog, "id" | "occurredAt">): Promise<TokenUsageLog> {
    const record = await this.prisma.tokenUsageLog.create({
      data: {
        model: entry.model,
  ...
  async findRecent(limit: number): Promise<TokenUsageLog[]> {
    const records = await this.prisma.tokenUsageLog.findMany({
