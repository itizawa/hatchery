...
export function createPrismaBatchRunLogRepository(prisma: PrismaClient): BatchRunLogRepository {
  return {
    async create(entry: Omit<BatchRunLog, "id" | "executedAt">): Promise<BatchRunLog> {
      const record = await prisma.batchRunLog.create({
        data: {
          status: entry.status,
    ...
    async findRecent(limit: number): Promise<BatchRunLog[]> {
      const records = await prisma.batchRunLog.findMany({
