...
- 「既知ワーカー」をスレッド参加者に限定するスコープ判断（§2）は、community 全体のワーカーへの言及（そのスレッドに登場していないワーカー）は検出されないというトレードオフを持つ。将来的に必要になれば `useCommunityWorkers` のロスターを渡す拡張で対応可能（`detectWorkerMentions` 側の変更は不要）。
