...
- 1 ページ目のレスポンスは pin 済み post 分（最大 `POST_PIN_MAX_COUNT` = 3 件）だけ `limit` を超える件数を返す（例: `limit=20` かつ pin 3 件なら最大 23 件）。`limit` を超えないよう通常枠を削るのではなく、pin 分を追加するデザインを採用した（cursor の意味・2 ページ目以降のページサイズを変えないため）。`posts.length <= limit` を前提にするクライアント・外部 API 利用者がいないことを確認済み（client は無限スクロールで件数を仮定していない）。
- 1 ページ目を先に取得しキャッシュした後、既存の cursor で 2 ページ目以降を取得している最中に admin が別の post を pin した場合、その post は（サーバ側では正しく除外・先頭表示の対象になるにもかかわらず）そのユーザーのセッションでは新しい pin としては見えず、フィードを最初から再読み込みするまで表示されない可能性がある。これは cursor ベースページネーションと「pin は 1 ページ目のみ」という設計の組み合わせに起因する既知の制約で、本 Issue のスコープでは対応しない（将来必要なら pin 変更の通知/キャッシュ無効化を別 Issue で検討する）。
- pin の上限チェック（community あたり `POST_PIN_MAX_COUNT` 件）は `pinPostIfUnderLimit`（`postRepository.ts`）で件数チェックと更新を単一の原子操作にしている（Prisma は SERIALIZABLE トランザクション + 直列化失敗時の `limit_exceeded` フォールバック）。これにより同時 pin リクエストによる上限超過（TOCTOU）を防いでいる。
- pin 済み post にも通常の post 一覧と同じ reveal フィルタ（`RevealFilterOptions.now`・ADR-0034 のドリップ配信）を適用する。admin が未公開（`createdAt` が未来）の post を pin しても、公開時刻が来るまではフィード先頭に表示されない。
