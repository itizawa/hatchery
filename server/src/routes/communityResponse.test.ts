...
describe("toCommunityResponse", () => {
  it("全フィールドを snake_case の公開 API 形式に変換する", () => {
    const res = toCommunityResponse({ record: baseRecord });
    expect(res).toEqual({
      id: "community-1",
      slug: "general",
      name: "General",
  ...
  it("synopsis が null のとき undefined（optional フィールド）として出力される", () => {
    const res = toCommunityResponse({ record: { ...baseRecord, synopsis: null } }) as Record<string, unknown>;
    expect(res.synopsis).toBeUndefined();
  });

  it("lastSlotKey が null のとき last_slot_key が undefined として出力される", () => {
    const res = toCommunityResponse({ record: { ...baseRecord, lastSlotKey: null } }) as Record<string, unknown>;
    expect(res.last_slot_key).toBeUndefined();
  });

  it("iconUrl が null のとき null のまま出力される（undefined にならない・#457）", () => {
    const res = toCommunityResponse({ record: { ...baseRecord, iconUrl: null } }) as Record<string, unknown>;
    expect(res.iconUrl).toBeNull();
    expect(res).toHaveProperty("iconUrl");
  });

  it("coverUrl が null のとき null のまま出力される（undefined にならない・#457）", () => {
    const res = toCommunityResponse({ record: { ...baseRecord, coverUrl: null } }) as Record<string, unknown>;
    expect(res.coverUrl).toBeNull();
    expect(res).toHaveProperty("coverUrl");
  });

  it("generationInstruction がレスポンスに含まれない（公開 API 漏洩防止・#488）", () => {
    const res = toCommunityResponse({ record: baseRecord }) as Record<string, unknown>;
  ...
  it("stats を渡すと post_count・last_post_at が反映される（#527）", () => {
    ...
    const res = toCommunityResponse({ record: baseRecord, stats });
  ...
  it("subscriberCount を渡すと subscriber_count が反映される（#930）", () => {
    const res = toCommunityResponse({ record: baseRecord, subscriberCount: 7 });
    expect(res.subscriber_count).toBe(7);
  });

  it("subscriberCount 省略時は subscriber_count=0（#930）", () => {
    const res = toCommunityResponse({ record: baseRecord });
    expect(res.subscriber_count).toBe(0);
  });
