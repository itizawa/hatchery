...
flowchart TD
    ...
    E --> F["🤖 AI: レビュー & 修正<br/>(/code-review)"]
    F -- 指摘あり --> F
    F -- 指摘なし = AI が develop へマージ --> H["👤 人間: develop → main へ昇格<br/>本番反映"]
```
...
| 担当 | やること |
|------|----------|
| 👤 人間 | Issue 作成、設計レビュー/承認、本番リリース判断 |
| 🤖 AI | 設計書作成、TDD実装、テスト、PR作成、レビュー・修正、develop へのマージ |
...
- 設計書と実装で **ブランチ・PR を分ける**。これにより「設計の承認（人間ゲート）」と「実装（AI が自走）」を独立させる。
...
2. Issue を `df:dev-review` に更新。

### フェーズ 6 — 🤖 AI: レビュー → 修正 → develop マージ
...
5. Issue に `df:done` を付与。
   - 設計どおりか不安が残る・自力で解消できない指摘がある場合は `df:blocked` を付け、人間に委ねる。
...
- `.github/workflows/df-review-fix.yml`: `df:dev-review` で起動 → レビュー → 修正 → 収束後に develop へマージ。
- 必要なもの:
...
5. **Step 4**: 実装・レビュー・develop マージも Actions 化。人間はゲート（設計承認 + 本番昇格）のみに。
