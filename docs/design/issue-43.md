...
- `client/src/config/env.ts` を新設し、Vite 公開環境変数を Zod で検証する。
  ...
  - 不正値はアプリ起動時（モジュール読込時）に `ZodError` を投げる。テスト容易性のため `loadClientEnv(source)` を export。
...
3. `VITE_API_BASE_URL` が **URL でない文字列**のとき `loadClientEnv` は `ZodError` を投げる（アプリ起動時に気付ける）。
