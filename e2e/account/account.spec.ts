...
/**
 * e2e/account/usecases.md の UC-ACCOUNT-01〜05 に対応する実テスト（Issue #738, #1102）。
...
test("UC-ACCOUNT-05: 初回ログイン直後（?welcome=1）に表示名設定を促す歓迎メッセージが表示される", async ({ page }) => {
  ...
  await expect(
    page.getByText("ようこそ Hatchery へ！まずは表示名を設定しましょう。"),
  ).toBeVisible();

  // `?welcome=1` が付かない通常の /account 表示では歓迎メッセージは表示されない（usecases.md の期待動作）
  await page.goto("/account");
  await expect(
    page.getByText("ようこそ Hatchery へ！まずは表示名を設定しましょう。"),
  ).not.toBeVisible();
