...
**自動選択は直近マイルストーンのみを対象とする。** 直近マイルストーン以外の Issue は引数なし `/df` の対象外。

候補の絞り込み手順:
1. **直近マイルストーンを決定する**: open Issue に付いた `milestone/*` ラベルのうち、アルファベット昇順で最小のものを「直近マイルストーン」とする（例: `milestone/v1.0.0` が `milestone/v1.1.0` より先）。
2. **候補 = open かつ直近マイルストーンのラベルが付いている** Issue のみ（closed / `milestone/*` 未設定 / 直近以外のマイルストーンは除外）。

**スキップ条件（フォールバックしない）**: 以下のいずれかに該当する場合、他マイルストーンへフォールバックせず「直近マイルストーンに着手可能な Issue が無いため対応をスキップします」と報告して終了する:
- `milestone/*` ラベル付きの open Issue が 1 件も存在しない
- 直近マイルストーンに AI 実行可能（フェーズ A または B）な Issue が無い

**優先順位**（候補が複数ある場合に上から順に評価し、最初に 1 件決まったら確定）:
1. **優先度ラベルの重み降順**（`priority/*` 無しは medium=2）。
2. 同優先度内では **フェーズ進捗の降順**（フェーズ B > フェーズ A。仕掛かりを先に片付ける）。
3. それでも同点なら **`createdAt` が古い順**（FIFO）。
