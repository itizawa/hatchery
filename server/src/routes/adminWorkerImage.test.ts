...
describe("POST /api/admin/workers/:id/image（#204）", () => {
  it("未認証なら 401 を返す", async () => {
    const app = await makeApp();
    const res = await request(app)
      .post("/api/admin/workers/haru/image")
  ...
  it("存在しない worker ID なら 404 を返す", async () => {
    const app = await makeApp();
    const agent = await loginAgent(app);
    const res = await agent
      .post("/api/admin/workers/nonexistent/image")
      .attach("image", Buffer.from("fake"), { filename: "avatar.png", contentType: "image/png" });
  ...
  it("ファイルが添付されていないなら 400 を返す", async () => {
    const app = await makeApp();
    const agent = await loginAgent(app);
    const res = await agent.post("/api/admin/workers/haru/image");
    expect(res.status).toBe(400);
  });
});
