...
import { test as testWrapper } from "../support/test.js";
...
test("UC-ACCOUNT-01: 表示名・プロフィール画像 URL を変更して保存できる", async ({ page }) => {
  ...
  // PATCH /api/auth/me は成功レスポンスを返す。後から登録したハンドラが優先されるため
  // setupAuthMocks の GET ハンドラはこの第2ハンドラにシャドウされる。GET と PATCH を
  // 両方このハンドラ内でメソッド分岐して処理する。
...
test("UC-ACCOUNT-04: プロフィール更新に失敗するとエラー内容が表示される", async ({ page }) => {
  ...
  // PATCH /api/auth/me を 500 エラーで返す。後から登録したハンドラが優先されるため
  // setupAuthMocks の GET ハンドラはこの第2ハンドラにシャドウされる。GET と PATCH を
  // 両方このハンドラ内でメソッド分岐して処理する。
  await page.route("**/api/auth/me", async (route) => {
    if (route.request().method() === "PATCH") {
...
// UC-ACCOUNT-05 は未実装スケルトン（usecases.md 参照）。support/test.ts のラッパー経由で
// test.todo() として登録し、実テスト（UC-ACCOUNT-01〜04）と同じファイル内に保持する。
testWrapper.todo("UC-ACCOUNT-05: 初回ログイン直後（?welcome=1）に表示名設定を促す歓迎メッセージが表示される");
