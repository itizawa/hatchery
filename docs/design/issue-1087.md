...
「tags は最大 5 個、各 30 文字以内。今回の話題を一言で表す言葉を選び、可能な範囲で頻出タグ
（直近の投稿で使われたタグ）を再利用してください」という指示を追記する。既存タグの再利用を促す
ため、`recentTitles` と同様に直近 post の `tags` を集約して重複除去した一覧
（`recentTags`・最大数十件を上限にする）をプロンプトに注入する。
...
- 関連投稿の「直近 N 件」の N はレスポンスサイズとのバランスで 5 件に固定する（Issue 本文に
  具体数の指定なし・要調整の余地あり）。
- タグの表記揺れ（同義語・全角半角等）の正規化は本 Issue のスコープ外（完全一致のみ）。
