...
test("UC-ADMIN-05: admin ユーザーが Worker を新規作成できる", async ({ page }) => {
  ...
  // 「ワーカーを追加」ボタンをクリック
  await page.getByRole("button", { name: "ワーカーを追加" }).click();
  const dialog = page.getByRole("dialog");
  await expect(dialog).toBeVisible();
  await expect(dialog.getByRole("heading", { name: "ワーカーを追加" })).toBeVisible();
...
test("UC-ADMIN-10: admin ユーザーがコミュニティのアイコン・カバー画像をアップロードできる", async ({ page }) => {
  ...
  // 「編集」をクリックして編集モーダルダイアログを開く（#833）
  await page.getByRole("button", { name: "編集" }).first().click();
  const dialog = page.getByRole("dialog");
  await expect(dialog).toBeVisible();

  // ダイアログ内に画像アップロード UI（アイコン画像テキスト）が現れる
  await expect(dialog.getByText("アイコン画像（クリックして変更）")).toBeVisible();

  // ファイル選択ダイアログをインターセプトしてアップロード
  const [fileChooser] = await Promise.all([
    page.waitForEvent("filechooser"),
    // アイコン画像（丸い Avatar）をクリックする
    dialog.getByAltText("テックニュース").last().click(),
  ]);
  ...
  // 編集ダイアログは引き続き表示されている（アップロードはフォーム送信と独立）
  await expect(dialog.getByText("アイコン画像（クリックして変更）")).toBeVisible();
});
...
test("UC-ADMIN-11: 管理画面タブのデータ取得に失敗すると再試行フォールバックが表示される", async ({ page }) => {
  await mockAdmin(page);
  await mockPublicCommunities(page);
  // GET /api/admin/batch-logs を 500 で失敗させる
  await page.route("**/api/admin/batch-logs", (route) =>
    route.fulfill({ status: 500, contentType: "application/json", body: JSON.stringify({ error: "Server Error" }) }),
  );

  await page.goto("/admin?tab=batch-logs");
  ...
  await expect(page.getByRole("heading", { name: "管理画面" })).toBeVisible();
  await expect(page.getByRole("tab", { name: "バッチログ" })).toBeVisible();
});
...
test("UC-ADMIN-14: admin ユーザーがコミュニティに生成プロンプト指示（非公開）を設定できる", async ({ page }) => {
  ...
  // 管理 API（PATCH /api/admin/communities/:id）: 生成プロンプト指示の更新（#833 編集ダイアログ）。
  // `*` は単一パスセグメントのみにマッチするため icon/cover アップロードのサブパスには影響しない。
  await page.route("**/api/admin/communities/*", (route) => {
    if (route.request().method() === "PATCH") {
      return route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({ id: "new-c", slug: "test-comm", name: "テストコミュニティ", description: "テスト用", generationInstruction: "脱さん付け・率直に話す。", created_at: "2024-01-01T00:00:00.000Z" }),
      });
    }
    return route.continue();
  });
  ...
  // 「コミュニティを追加」ボタンを押して作成モーダルダイアログを開く（#833）
  await page.getByRole("button", { name: "コミュニティを追加" }).click();
  const addDialog = page.getByRole("dialog");
  await expect(addDialog).toBeVisible();

  // ダイアログ内の作成フォームに入力（slug・名前・概要・生成プロンプト指示）
  await addDialog.getByLabel("slug（URL 識別子）").fill("test-comm");
  await addDialog.getByLabel("コミュニティ名", { exact: true }).fill("テストコミュニティ");
  await addDialog.getByLabel("コミュニティ概要（公開）").fill("テスト用");
  await addDialog.getByLabel(/生成プロンプト指示/).fill("率直に話す。");

  // 「作成」ボタンをクリック
  await addDialog.getByRole("button", { name: "作成" }).click();
  ...
  // ステップ5: 一覧の「編集」を開き、生成プロンプト指示欄に作成時の値が初期表示される（#833 編集ダイアログ）。
  await page.getByRole("button", { name: "編集" }).first().click();
  const editDialog = page.getByRole("dialog");
  await expect(editDialog).toBeVisible();
  await expect(editDialog.getByLabel(/生成プロンプト指示/)).toHaveValue("率直に話す。");

  // ステップ6: 指示文を書き換えて保存する → 更新 API が変更後の値で呼ばれダイアログが閉じる。
  await editDialog.getByLabel(/生成プロンプト指示/).fill("脱さん付け・率直に話す。");
  const [patchRequest] = await Promise.all([
    page.waitForRequest(
      (r) => r.url().includes("/api/admin/communities/") && r.method() === "PATCH",
    ),
    editDialog.getByRole("button", { name: "保存" }).click(),
  ]);
  expect(patchRequest.postDataJSON()).toMatchObject({ generationInstruction: "脱さん付け・率直に話す。" });
  await expect(page.getByRole("dialog")).toHaveCount(0);
});

test("UC-ADMIN-15: admin ユーザーが Worker の文章量（verbosity）を編集できる（#625）", async ({ page }) => {
  const WORKER_ID = "w1";
  // verbosity 未設定ワーカー（初期値は「標準」になる）
  let savedVerbosity = "standard";
  await mockAdmin(page);
  await mockPublicCommunities(page);
  await mockAdminWorkers(page, [{ id: WORKER_ID, displayName: "テストワーカーA", role: "エンジニア" }]);
  // 参加コミュニティ取得
  ...
  // WorkerCommunitiesField
  await mockAdminCommunities(page, []);
  // PATCH /api/workers/:id: verbosity を更新し、以降の GET で更新後の値を返す
  await page.route(`**/api/workers/${WORKER_ID}`, (route) => {
    if (route.request().method() === "PATCH") {
      const body = route.request().postDataJSON() as { verbosity?: string };
      if (body?.verbosity) savedVerbosity = body.verbosity;
      return route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({ id: WORKER_ID, displayName: "テストワーカーA", role: "エンジニア", verbosity: savedVerbosity }),
      });
    }
    return route.continue();
  });
  // GET /api/workers: 保存後は更新済み verbosity を返す
  await page.route("**/api/workers", (route) => {
    if (route.request().method() !== "GET") return route.continue();
    return route.fulfill({
      status: 200,
  ...
  // verbosity 未設定ワーカーは「標準」が初期選択になる
  const verbositySelect = dialog.getByRole("combobox", { name: /文章量/ });
  await expect(verbositySelect).toHaveValue("standard");

  // 「詳細」を選択する
  await verbositySelect.click();
  await page.getByRole("option", { name: "詳細" }).click();

  // 「保存」ボタンをクリックし PATCH が verbosity: "detailed" を含む形で呼ばれる
  const [patchRequest] = await Promise.all([
    page.waitForRequest((r) => r.url().includes(`/api/workers/${WORKER_ID}`) && r.method() === "PATCH"),
    saveButton.click(),
  ]);
  expect(patchRequest.postDataJSON()).toMatchObject({ verbosity: "detailed" });

  // 保存成功 → ダイアログが閉じる
  await expect(dialog).not.toBeVisible();

  // 再度「編集」ダイアログを開くと「詳細」が選択状態で表示される（永続化の確認）
  await page.getByRole("button", { name: "編集 テストワーカーA" }).click();
  const reopenedDialog = page.getByRole("dialog");
  await expect(reopenedDialog).toBeVisible();
  await expect(reopenedDialog.getByRole("button", { name: "保存" })).toBeEnabled({ timeout: 5000 });
  await expect(reopenedDialog.getByRole("combobox", { name: /文章量/ })).toHaveValue("detailed");
});
