...
- AC-6: `Employee` 型に `isBot` が含まれる（型レベル。既存フィクスチャを壊さないため型上は任意）。
...
`isBot: z.boolean().default(false)` を追加する。エクスポートする `Employee` 型は **`z.input`** から導出し、
`isBot` を型上は任意にする。理由: `.default(false)` で `z.infer`（output）は `isBot: boolean`（必須）となり、
他 Issue（#25/#32/#33）の既存テストフィクスチャ（`{ id, displayName }` のみ）が型エラーになる。
スキーマの `.default(false)` により `parse` 後は常に `boolean` が入るため実行時の保証は保たれる。
`DEFAULT_EMPLOYEES` は AI 社員なので各要素に `isBot: true` を明示する。
...
common の `DEFAULT_EMPLOYEES` を単一情報源として AI 社員を upsert（`isBot=true / userId=null`）。
ログインユーザー（既存 seed の `testuser`）に対応する Employee を `isBot=false / userId=testuser` で upsert。

## 5. 影響範囲 / 既存への変更

- **common**: `domain/employee/employee.ts`（EmployeeSchema / 型 / DEFAULT_EMPLOYEES）、`domain/auth/auth.ts`（AuthUserSchema）。
- **server**: `prisma/schema.prisma`、新規マイグレーション、`persistence/userRepository.ts`（User 型 / InMemory）、`persistence/prismaUserRepository.ts`、`auth/passport.ts`、`prisma/seed.ts`。
...
- AC-12（seed）は DB 必須のため統合テスト（`describe.skipIf(!DATABASE_URL)`）で isBot 既定・userId 一意 1:1 を検証。CI ではスキップ。
