...
describe("HomeFeedScene — 人気順固定 (#1067)", () => {
  beforeEach(() => {
    vi.restoreAllMocks();
    // #932: 再訪問ユーザー相当の localStorage 状態にして showWelcome を posts の有無のみに依存させる。
    const lsMock = makeLocalStorageMock();
  ...
  it("/ では GET /api/feed?sort=popular が呼ばれる", async () => {
    const fetchMock = vi.fn().mockImplementation((input: RequestInfo | URL) => {
      const url = input instanceof Request ? input.url : String(input);
      if (url.includes("/auth/me")) {
        return Promise.resolve(new Response(null, { status: 401 }));
...
describe("HomeFeedScene — 投稿カードからスレッドへ遷移 (#498)", () => {
  ...
  it("投稿カードのタイトルをクリックするとスレッド（/posts/$postId）へ遷移する", async () => {
    ...
    expect(screen.queryByRole("heading", { name: "人気の投稿" })).not.toBeInTheDocument();
  });
});
...
describe("HomeFeedScene — 所属コミュニティ名表示（混在フィード・#503）", () => {
  ...
  beforeEach(() => {
    vi.restoreAllMocks();
    // #932: 再訪問ユーザー相当の localStorage 状態にして showWelcome=false にする。
    // hasVisited=false のままだと WelcomeSection も同時に描画されコミュニティ名 chip の検出が不安定になる。
    const lsMock = makeLocalStorageMock();
    lsMock.setItem("hatchery_visited", "true");
    vi.stubGlobal("localStorage", lsMock);
  });
...
describe("HomeFeedScene — ゲストも vote できる (#777)", () => {
  ...
  beforeEach(() => {
    vi.restoreAllMocks();
    // #932: 再訪問ユーザー相当の localStorage 状態にして showWelcome=false にする。
    // hasVisited=false のままだと WelcomeSection も同時に描画され vote ボタンの検出が不安定になる。
    const lsMock = makeLocalStorageMock();
    lsMock.setItem("hatchery_visited", "true");
    vi.stubGlobal("localStorage", lsMock);
  });
