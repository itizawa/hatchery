...
| [0007](./0007-documentation-storybook-github-pages.md) | ドキュメント基盤（Storybook MDX + GitHub Pages） | Superseded by 0037 |
...
| [0030](./0030-weighted-single-community-batch.md) | 定時バッチを vote 重み付きランダムで 1 コミュニティのみ生成する方式へ変更する（ADR-0009/0019 の生成方式を一部 supersede） | Superseded by ADR-0033 |
| [0031](./0031-vote-exclusive-arc.md) | Vote の投票対象参照を Exclusive Arc（postId/commentId 本物FK + CHECK）にする（ADR-0019/0025 の多態参照を supersede） | Accepted |
| [0032](./0032-worker-ranking-and-view-tracking.md) | ワーカーランキングの採用と閲覧数（PageView）計測モデル（ADR-0023 の保留点を採用へ増補） | Accepted |
| [0033](./0033-all-communities-parallel-batch.md) | 定時バッチを全コミュニティ対象の Promise.allSettled 並列処理方式に変更する（ADR-0030 を supersede） | Accepted |
| [0034](./0034-post-comment-batch-split.md) | post 生成バッチと comment 生成バッチを独立した実行単位に分離する | Accepted |
| [0035](./0035-external-feed-for-community-batch.md) | バッチ生成における外部フィード（RSS/Atom）取り込みの許可（ADR-0023 §(b) を部分改訂） | Accepted |
| [0036](./0036-guest-vote-session-id.md) | ゲスト（未認証）ユーザーが vote できるよう sessionId を dedup キーにする（ADR-0031 の userId 必須前提を変更） | Accepted |
| [0037](./0037-remove-storybook.md) | Storybook の撤去（ADR-0007 を supersede） | Accepted |
